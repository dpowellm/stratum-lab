...
timeout $((SMOKE_TIMEOUT + 120)) docker run --rm \
    --network=host \
    --entrypoint bash \
    --memory=4g \
...
if [ "$SMOKE_OK" = true ]; then
    ...
    echo "  SMOKE TEST FAILED — debug files preserved:"
    echo "    cat $SMOKE_DIR/container.log"
    echo "    cat $SMOKE_DIR/status.txt"
    echo "    head -5 $SMOKE_DIR/events_run_1.jsonl"
    echo ""
...
echo ""
echo "  After the scan completes, validate data quality:"
echo ""
echo "    python3 $DROPLET_DIR/scripts/validate_scan.py ~/scan_output/results \\"
echo "        --output ~/scan_output/validation_report.json"
echo ""
